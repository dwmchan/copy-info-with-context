...
exports.getSensitiveColumnPatterns = exports.detectColumnAlignments = exports.buildAsciiTable = exports.getColumnRangeFromSelection = exports.detectHeaders = exports.detectColumnType = exports.shouldMaskColumn = exports.parseCsvLine = exports.detectDelimiter = void 0;
/**
 ...
 */
function shouldMaskColumn(columnName, config) {
    ...
    for (const [category, patterns] of Object.entries(SENSITIVE_COLUMN_PATTERNS)) {
        if (config.types[category] === false) {
            continue;
        }
...
exports.shouldMaskColumn = shouldMaskColumn;
/**
 ...
 */
function detectHeaders(firstLine, secondLine, delimiter = ',') {
...
function getColumnRangeFromSelection(line, startChar, endChar, delimiter = ',') {
 ...
 */
function buildAsciiTable(headers, rows, columnWidths) {
    // Calculate column widths if not provided
    const widths = columnWidths ?? headers.map((header, i) => {
        const headerLen = header.length;
        const maxDataLen = Math.max(...rows.map(row => (row[i] ?? '').toString().length));
    ...
    const bottomLine = chars.bottomLeft +
        ...
        chars.bottomRight;
    // Build header row
    const headerRow = `${chars.vertical} ${headers.map((h, i) => h.padEnd(widths[i] ?? 0)).join(` ${chars.vertical} `)} ${chars.vertical}`;
    // Build data rows
    const dataRows = rows.map(row => `${chars.vertical} ${row.map((cell, i) => (cell || '').toString().padEnd(widths[i] ?? 0)).join(` ${chars.vertical} `)} ${chars.vertical}`);
    // Combine all parts
    return [
        topLine,
        headerRow,
...
function detectColumnAlignments(rows) {
    ...
    for (let col = 0; col < numColumns; col++) {
        ...
        const booleanCount = values.filter(v => ['true', 'false', 'yes', 'no', '1', '0'].includes(v.toLowerCase())).length;
        // Determine alignment
 ...
 */
function getSensitiveColumnPatterns() {
    return SENSITIVE_COLUMN_PATTERNS;
}
exports.getSensitiveColumnPatterns = getSensitiveColumnPatterns;
