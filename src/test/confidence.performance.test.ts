...
describe('isInsideFieldName() - Performance Tests', () => {
    ...
    test('performance comparison - short vs long context', () => {
        ...
        for (let i = 0; i < iterations; i++) {
            isInsideFieldName(shortXml, shortTextIndex, 3);
        }
        ...
        for (let i = 0; i < iterations; i++) {
            isInsideFieldName(longXml, longTextIndex, 3);
        }
    ...
    <bsb>345-678</bsb>
        ...
        const startTime = performance.now();
        for (let i = 0; i < iterations; i++) {
            isInsideFieldName(worstCaseXml, textIndex, 3);
        }
        const endTime = performance.now();
    ...
    <accountNumber>888999777</accountNumber>
        ...
        for (let i = 0; i < iterations; i++) {
            for (const tagIndex of bsbTags) {
                isInsideFieldName(largeXml, tagIndex, 3);
            }
        }
        const endTime = performance.now();
